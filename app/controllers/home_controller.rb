  ...
  before_filter :authenticate_user!, :before_query
	...
	def index
		if current_user.has_role? :admin
			@past_events = Event.past.paginate(page: params[:page], per_page: 10)
			@upcoming_events = Event.upcoming.paginate(page: params[:page], per_page: 10)
		else
			@past_events = Event.where(school_email: current_user.email).past.paginate(page: params[:page], per_page: 10)
			@upcoming_events = Event.where(school_email: current_user.email).upcoming.paginate(page: params[:page], per_page: 10)
		end
